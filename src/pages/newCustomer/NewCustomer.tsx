...
function NewCustomer() {
  ...
  useEffect(() => {
    // אפשר לבצע פעולות ב-useEffect אם נדרש
  }, []);

  const handleNextStep = (lastStatement: boolean) => {
    if (lastStatement) {
      addStatementsToDB();
    }
  ...
  const renderStepComponent = () => {
    switch (steps[currentStep]) {
      case "PersonalDetails":
        return <PersonalDetailsForm onNext={() => handleNextStep(false)} customerEmail={setCustomerEmail} />;
      case "statement1":
        return (
          <ConsentForm
      ...
      case "healthStatement":
        return (
          <div className={styles.healthStatementContainer}>
            <HealthStatement onNext={() => handleNextStep(false)} customerEmail={customerEmail} />
          </div>
        );
      case "confirm":
        return (
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.content}>{renderStepComponent()}</div>
    </div>
  );
}
